...
<%	
	...
	if (pwCk.equals("false")) {	// 이전패스워드 확인 실패
		...
		String deletePageOpen = "false"; 
		response.sendRedirect(request.getContextPath()+"/myPageDelete.jsp?msg="+msg);
	}else if(pwCk.equals("true")){	// 이전패스워드 확인 성공
		String deletePageOpen = "true"; 
		response.sendRedirect(request.getContextPath()+"/myPageDelete.jsp?msg="+""+"&deletePageOpen="+deletePageOpen);
